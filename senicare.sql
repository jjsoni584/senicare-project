...
CREATE TABLE IF NOT EXISTS `senicare`.`tel_auth_number` (
  ...
  `auth_numbercol` VARCHAR(4) NOT NULL COMMENT '인증번호',
...
CREATE TABLE IF NOT EXISTS `senicare`.`tools` (
  ...
  `count` INT NOT NULL COMMENT '용품 잔여수량',
...
CREATE TABLE IF NOT EXISTS `senicare`.`care_records` (
  `record_number` INT NOT NULL AUTO_INCREMENT COMMENT '관리기록 번호',
  `record_date` DATE NOT NULL COMMENT '관리기록 날짜',
  `contents` TEXT NOT NULL COMMENT '관리 내용',
  `used_tool` INT NULL COMMENT '관리에 사용한 물품',
  `count` INT NULL COMMENT '관리시 사용한 용품 갯수',
  ...
  INDEX `cared_custom_idx` (`custom_number` ASC) VISIBLE,
...
CREATE USER 'senicare'@'%' IDENTIFIED BY 'JJ1234';
